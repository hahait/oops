...
class ZabbixItemModel(models.Model):
    item = models.CharField("监控项名称", max_length=200, null=True, unique=True, db_index=True, help_text="监控项名称")
    item_name = models.CharField("监控项别名", max_length=200, null=True, unique=True, help_text="每个监控项别名")
    item_key = models.CharField("监控项 key", max_length=250, null=True, db_index=True, unique=True, help_text="zabbix 数据库中的 item key")
    app_name = models.ManyToManyField(AppConfigModel, null=True, blank=True, verbose_name="应用名", help_text="关联的应用名")
    ...
    class Meta:
        verbose_name = "应用与监控项的关联关系"
        db_table = "monitor_zabbix_item_app"

class ZabbixHostItemModel(models.Model):
    server = models.ForeignKey(ServerModel, null=True, verbose_name="关联的服务器",help_text="关联的服务器")
    hostid = models.BigIntegerField("服务器对应的 hostid",db_index=True,help_text="服务器对应的 hostid")
    itemid = models.BigIntegerField("hostid 对应的多个 itemid",db_index=True,help_text="hostid 对应的多个 itemid")
    item_key = models.CharField("每个监控项的key", max_length=255, null=True,db_index=True, help_text="每个监控项的key")
    item_name = models.CharField("每个监控项名称",max_length=300,null=True,help_text="每个监控项名称")
    item_units = models.CharField("每个监控值的单位",max_length=10,null=True,help_text="每个监控值的单位")
    item_delay = models.CharField("每个监控项的监控间隔", max_length=100, null=True, help_text="每个监控项的监控间隔")
    value_type = models.IntegerField("每个 item 监控值的类型",help_text="每个 item 监控值的类型")
    history_model_name = models.CharField("每个 item 监控值存储的 history 表的模型名称",max_length=100,null=True,help_text="每个 item 监控值存储的 history 表的模型名称")
    trends_model_name = models.CharField("每个 item 监控值存储的 trends 表的模型名称",max_length=100,null=True,help_text="每个 item 监控值存储的 trends 表的模型名称")
    create_time = models.DateTimeField("添加时间", auto_now_add=True, null=True, help_text="添加时间")

    def __str__(self):
        return "%s: %s" %(self.server.manager_ip,self.hostid)

    class Meta:
        verbose_name = "服务器与 Zabbix 中 hostid/itemid/hostory_table/trends_table 的关联关系"
        db_table = "monitor_zabbix_host_item"

class MonitorAlertModel(models.Model):
    STATUS_CHOICES = (
        ("OK", "OK"),
        ("PROBLEM", "PROBLEM")
    )
    ...
    title = models.CharField("告警标题",max_length=100, null=False, blank=False, help_text='告警标题')
    detail = models.TextField('告警内容',max_length=1000, null=True,help_text="告警内容")
    eventid = models.CharField("告警事件ID",max_length=100, null=True, blank=True, help_text='告警事件ID,用于查询告警后是否恢复')
    source = models.CharField("告警来源",max_length=10,default="zabbix",help_text="告警来源")
    trigger_name = models.CharField("告警触发器名称",max_length=200, null=True, blank=True, help_text='告警触发器名称,用于对告警进行统计')
    level = models.CharField("告警级别",choices=LEVEL_CHOICES,max_length=20,default="Info",help_text="告警级别")
    server = models.ForeignKey(ServerModel, null=True, blank=True, verbose_name="关联的服务器", help_text="关联的服务器")
    status = models.CharField("告警状态",choices=STATUS_CHOICES,max_length=10,default="OK",help_text="告警当前的状态")
    start_time = models.DateTimeField("告警产生时间", null=True, blank=True, help_text="告警产生时间")
    end_time = models.DateTimeField("告警结束时间",null=True, blank=True, help_text='告警结束时间')
    create_time = models.DateTimeField("添加时间", auto_now_add=True, null=True, help_text="添加时间")
    last_update_time = models.DateTimeField("最后一次更新时间", auto_now=True, null=True, help_text="最后一次修改时间")
    ...
    class Meta:
        verbose_name = "监控告警统计"
        db_table = "monitor_monitor_alert"
...
class History(models.Model):
    itemid = models.ForeignKey("Items",models.DO_NOTHING,db_column="itemid",related_name="item",blank=True,null=True)
    clock = models.IntegerField()
...
class HistoryLog(models.Model):
    itemid = models.ForeignKey("Items", models.DO_NOTHING, db_column="itemid", related_name="log_item", blank=True,null=True)
    clock = models.IntegerField()
...
class HistoryStr(models.Model):
    itemid = models.ForeignKey("Items", models.DO_NOTHING, db_column="itemid", related_name="str_item", blank=True,null=True)
    clock = models.IntegerField()
    value = models.CharField(max_length=255)
    ns = models.IntegerField(primary_key=True)
...
class HistoryText(models.Model):
    itemid = models.ForeignKey("Items", models.DO_NOTHING, db_column="itemid", related_name="text_item", blank=True,
                               null=True)
    clock = models.IntegerField()
    value = models.TextField()
    ns = models.IntegerField(primary_key=True)
...
class HistoryUint(models.Model):
    itemid = models.ForeignKey("Items", models.DO_NOTHING, db_column="itemid", related_name="uint_item", blank=True,
                               null=True)
    clock = models.IntegerField()
